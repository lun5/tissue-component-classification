...
while i<numel(varargin)
    ...
    val = varargin{i};
    switch val
        case {'-colours','-colors','-colour','-color'}; opt.node_col = varargin{i+1}; i = i+1;
...
for i=1:numel(opt.deps.from)
    str{end+1} = sprintf('%s -> %s;',...
        opt.names{opt.deps.from(i)},...
        opt.names{opt.deps.to(i)});
end
str{end+1} = '}';
...
for i=1:numel(nodes)
    ...
    [tf,loc] = ismember(opt.node(i).text,opt.names);
    if ~tf; keyboard; end
    opt.node(i).index = loc;
    opt.node(i).color = opt.node_col(loc);
    ...
    if true
        opt.node(i).handle_box = rectangle(...
            ...
            'EdgeColor',opt.node_col(opt.node(i).index));
        ...
        opt.node(i).handle_box = plot(...
            opt.node(i).x + opt.node(i).w*.5*[-1 -1 1 1 -1],...
            opt.node(i).y + opt.node(i).h*.5*[-1 1 1 -1 -1],...
            'color',opt.node_col(opt.node(i).index));
    end
end
...
if ~isnumeric(opt.fontsize)
    for i=1:numel(opt.node)
        ...
        fscale = min([fscale pos_box(3)/pos_txt(3) pos_box(4)/pos_txt(4)]);
        
    end
    %round down to nearest .5, do not go below 6 pint
...
for i=1:numel(edges)
    ...
    [tf,loc] = ismember(opt.edge(i).endpoints,opt.names);
